...
# -*- coding: utf-8 -*-
"""
Исправленный GUI интерфейс с улучшенной обработкой ошибок.
"""
...
class NormsAnalyzerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Анализатор норм расхода электроэнергии РЖД (исправленная версия)")
        ...
        logger.info("GUI инициализирован с исправлениями")
    ...
    def _check_dependencies(self):
        ...
        if missing_deps:
            error_msg = f"Отсутствуют необходимые библиотеки: {', '.join(missing_deps)}"
    ...
    def setup_logging(self):
        ...
        class GUILogHandler(logging.Handler):
            ...
            def _append_log(self, msg, level):
                try:
                    ...
                    if len(lines) > 1000:  # Максимум 1000 строк
                        self.text_widget.delete("1.0", f"{len(lines)-500}.0")
        ...
        try:
            ...
            print(f"Ошибка настройки логирования GUI: {e}")
    ...
    def setup_styles(self):
        ...
        try:
            ...
            logger.warning(f"Ошибка настройки стилей: {e}")
    ...
    def select_route_html_files(self):
        ...
        try:
            ...
            logger.error(f"Ошибка выбора файлов маршрутов: {e}")
            messagebox.showerror("Ошибка", f"Ошибка выбора файлов: {e}")
    ...
    def select_norm_html_files(self):
        ...
        try:
            ...
            logger.error(f"Ошибка выбора файлов норм: {e}")
            messagebox.showerror("Ошибка", f"Ошибка выбора файлов: {e}")
    ...
    def clear_route_files(self):
        ...
        try:
            ...
            logger.error(f"Ошибка очистки файлов маршрутов: {e}")
    ...
    def clear_norm_files(self):
        ...
        try:
            ...
            logger.error(f"Ошибка очистки файлов норм: {e}")
    ...
    def load_routes(self):
        ...
        if self.analyzer is None:
            messagebox.showerror("Ошибка", "Анализатор не инициализирован")
            return
    ...
    def _update_norms_load_status(self, success: bool):
        ...
        try:
            ...
            logger.error(f"Ошибка обновления статуса загрузки норм: {e}")
            self.load_norms_button.config(state='normal')
    ...
    def on_section_selected(self, event=None):
        ...
        try:
            ...
            logger.error(f"Ошибка выбора участка: {e}")
    ...
    def on_norm_selected(self, event=None):
        ...
        try:
            ...
            logger.error(f"Ошибка выбора нормы: {e}")
    ...
    def _extract_norm_id_from_text(self, norm_text: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Ошибка извлечения ID нормы: {e}")
            return None
    ...
    def show_norm_info(self):
        ...
        try:
            ...
            if not norm_id or self.analyzer is None:
                return
            ...
            logger.error(f"Ошибка показа информации о норме: {e}")
            messagebox.showerror("Ошибка", f"Ошибка отображения информации: {e}")
    ...
    def analyze_section(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Ошибка запуска анализа: {e}")
    ...
    def _update_analysis_results(self, fig, statistics, error, section_name: str, norm_id: Optional[str], single_section_only: bool):
        ...
        try:
            # Включаем кнопку анализа обратно
            self.analyze_button.config(state='normal')
            ...
            logger.error(f"Ошибка обновления результатов анализа: {e}")
            self.analyze_button.config(state='normal')
    ...
    def open_locomotive_filter(self):
        ...
        try:
            ...
            logger.error(f"Ошибка открытия фильтра локомотивов: {e}")
            messagebox.showerror("Ошибка", f"Ошибка фильтра: {e}")
    ...
    def edit_norms(self):
        ...
        try:
            ...
            logger.error(f"Ошибка редактирования норм: {e}")
    ...
    def update_plot_info(self, section_name: str, stats: Dict, norm_id: Optional[str] = None, single_section_only: bool = False):
        ...
        try:
            ...
            logger.error(f"Ошибка обновления информации о графике: {e}")
    ...
    def update_plot_info_default(self):
        ...
        try:
            ...
            text += "✓ Исправлены ошибки с Plotly scatter plots\n"
            text += "✓ Улучшена обработка ошибок\n"
            text += "✓ Добавлена проверка зависимостей\n"
            text += "✓ Исправлены проблемы с типами данных\n\n"
            ...
            logger.error(f"Ошибка обновления информации по умолчанию: {e}")
    ...
    def open_plot_in_browser(self):
        ...
        try:
            ...
            logger.error(f"Ошибка открытия графика: {e}")
            messagebox.showerror("Ошибка", f"Ошибка открытия: {e}")
    
    def export_to_excel(self):
        """Экспортирует данные в Excel с полным форматированием"""
        try:
            if self.analyzer is None:
                messagebox.showwarning("Предупреждение", "Анализатор не инициализирован")
                return
    ...
    def show_norm_storage_info(self):
        ...
        try:
            ...
            info += f"Общее количество норм: {storage_info.get('total_norms', 0)}\n"
            ...
            # Диапазоны значений
            load_range = norm_stats.get('load_ranges', {})
            consumption_range = norm_stats.get('consumption_ranges', {})
    ...
    def show_routes_statistics(self):
        ...
        try:
            ...
            content += f"Маршрутов найдено: {processing_stats.get('total_routes_found', 0)}\n"
            content += f"Уникальных маршрутов: {processing_stats.get('unique_routes', 0)}\n"
            content += f"Дубликатов удалено: {processing_stats.get('duplicates_total', 0)}\n"
            content += f"Обработано успешно: {processing_stats.get('routes_processed', 0)}\n"
            content += f"Пропущено: {processing_stats.get('routes_skipped', 0)}\n"
            content += f"Итоговых записей: {processing_stats.get('output_rows', 0)}\n\n"
            
            # Статистика по участкам
            if 'Наименование участка' in routes_data.columns:
            ...
            if 'Номер нормы' in routes_data.columns:
            ...
            logger.error(f"Ошибка показа статистики маршрутов: {e}")
            messagebox.showerror("Ошибка", f"Ошибка отображения статистики: {e}")
